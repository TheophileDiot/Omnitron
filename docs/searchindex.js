Search.setIndex({
  docnames: ["commands", "index"],
  envversion: {
    "sphinx.domains.c": 2,
    "sphinx.domains.changeset": 1,
    "sphinx.domains.citation": 1,
    "sphinx.domains.cpp": 4,
    "sphinx.domains.index": 1,
    "sphinx.domains.javascript": 2,
    "sphinx.domains.math": 2,
    "sphinx.domains.python": 3,
    "sphinx.domains.rst": 2,
    "sphinx.domains.std": 2,
    sphinx: 56,
  },
  filenames: ["commands.rst", "index.rst"],
  objects: {},
  objnames: {},
  objtypes: {},
  terms: {
    0: 0,
    1: 0,
    10: 0,
    2: 0,
    20: 0,
    25: 0,
    2562: 1,
    99: 0,
    case: 1,
    default: 0,
    long: 0,
    new: 0,
    In: 0,
    It: 0,
    The: 0,
    about: 0,
    acc: 0,
    activ: [0, 1],
    add: 0,
    alias: 0,
    all: 0,
    allow: 0,
    also: 0,
    amount: 0,
    an: 0,
    announc: 0,
    application_id: 0,
    ar: 0,
    artist: 0,
    attach: 0,
    awkw: 0,
    awkword: 0,
    b_io: 0,
    ban: 0,
    batgregate900: 1,
    been: 0,
    befor: 0,
    betray: 0,
    betrayal_io: 0,
    bonu: 0,
    boost: 0,
    bot: [0, 1],
    can: 0,
    categori: 0,
    certain: 0,
    cfg: 0,
    channel: [0, 1],
    check: 0,
    checker: 0,
    chess: 0,
    chess_in_the_park: 0,
    chkr: 0,
    choic: 0,
    clear: 0,
    cmds_chan: 0,
    come: 1,
    command_channel: 0,
    commands_channel: 0,
    commands_count: 0,
    complet: 0,
    config: 0,
    configur: [0, 1],
    connect: 0,
    contain: 0,
    count: 0,
    creat: 0,
    crew: 0,
    cstm: 0,
    current: 0,
    custom: 0,
    d: 0,
    d_crew: 0,
    dai: 0,
    del: 0,
    delet: [0, 1],
    descript: 0,
    detail: 0,
    develop: 1,
    disconnect: 0,
    discord: 1,
    displai: 0,
    dj: 1,
    doodl: 0,
    doodle_crew: 0,
    durat: 0,
    duration_typ: 0,
    duration_valu: 0,
    dure: 0,
    e: 1,
    end: 0,
    endticket: 0,
    eras: 0,
    error: 0,
    et: 0,
    event: 0,
    everi: [0, 1],
    everyon: 0,
    everywher: 0,
    execut: 0,
    experi: 0,
    f_io: 0,
    fals: 0,
    featur: 0,
    finish: 0,
    fishington: 0,
    fishington_io: 0,
    format: 0,
    from: 0,
    full: 0,
    futur: 1,
    g: 1,
    gain: 0,
    get: 0,
    given: 0,
    group: 1,
    h: 0,
    ha: 0,
    halp: 0,
    handl: 1,
    have: 1,
    help: 0,
    high: 1,
    hour: 0,
    how: 0,
    id_of_the_poll_messag: 0,
    identifi: 0,
    includ: 0,
    info: 0,
    inform: 0,
    insensit: 1,
    insid: 0,
    instant: 0,
    invit: 0,
    io: 0,
    join: 0,
    just: 0,
    kick: 0,
    l_2_r: 0,
    l_tile: 0,
    latenc: 0,
    leaderboard: 0,
    leav: 0,
    letter: 0,
    letter_til: 0,
    level: 0,
    level_2_rol: 0,
    level_to_rol: 0,
    link: 0,
    list: 0,
    listen: 0,
    ll: 0,
    lvl: 0,
    lvl_2_role: 0,
    lvl_to_rol: 0,
    m: 0,
    m_on_j: 0,
    manag: 0,
    max: 0,
    max_lvl: 0,
    me: 0,
    member: 0,
    members_count: 0,
    messag: 0,
    messages_count: 0,
    min: 0,
    minimum: 0,
    minut: 0,
    miscellan: 1,
    mod: 0,
    mod_channel: 0,
    moder: 1,
    mods_channel: 0,
    more: [0, 1],
    multipl: 0,
    music: 0,
    music_chan: 0,
    music_channel: 0,
    mute: 0,
    mute_on_join: 0,
    mute_rol: 0,
    muted_rol: 0,
    mx_lvl: 0,
    name: 0,
    next: 0,
    night: 0,
    none: 0,
    notifi: 0,
    number: 0,
    off: 0,
    one: 0,
    onli: 0,
    other: 0,
    our: 0,
    own: 0,
    owner: 1,
    p_n: 0,
    park: 0,
    particular: 0,
    pass: 0,
    passag: 0,
    paus: 0,
    ping: 0,
    plai: 0,
    player: 0,
    playlist: 0,
    poker: 0,
    poker_night: 0,
    poll: 0,
    polls_chan: 0,
    polls_channel: 0,
    posit: 0,
    possibl: 0,
    precis: 0,
    prefix: [0, 1],
    prematur: 0,
    prestg: 0,
    prestig: 0,
    prestige_valu: 0,
    prev_i: 0,
    prevent_invit: 0,
    prfx: 0,
    procedur: 0,
    progress: 0,
    project: 1,
    prstg: 0,
    purg: 0,
    purpos: 1,
    queue: 0,
    rank: 0,
    reason: 0,
    relat: 0,
    remain: 0,
    remov: 0,
    requir: 0,
    reset: 0,
    resolv: 0,
    retriev: 0,
    role: 0,
    s_a: 0,
    sai: 0,
    salon: 0,
    sanction: 0,
    sc_p: 0,
    second: 0,
    secu: 0,
    secur: 0,
    select2rol: 0,
    select2role_channel: 0,
    select: 0,
    select_2_rol: 0,
    select_to_rol: 0,
    select_to_role_channel: 0,
    send: 0,
    sent: 0,
    server: [0, 1],
    server_2_rol: 0,
    serverinfo: 0,
    set: 0,
    show: 0,
    si: 0,
    sketchi: 0,
    sketchy_artist: 0,
    skip: 0,
    slash: 1,
    smart: 1,
    snack: 0,
    someon: 0,
    song: 0,
    soundcloud: 0,
    special: 0,
    specif: 0,
    specifi: 0,
    spell: 0,
    spellcast: 0,
    stat: 0,
    state: 0,
    stop: 0,
    strike: 0,
    sub: 0,
    support: 0,
    talk: 0,
    text: 0,
    text_channel: 0,
    than: 0,
    thei: 0,
    thi: [0, 1],
    ticket: 0,
    tile: 0,
    time: 0,
    titl: 0,
    togeth: 0,
    top: 0,
    tri: 0,
    turn: 0,
    type: 0,
    type_of_dur: 0,
    ui: 0,
    unban: 0,
    under: 1,
    unlik: 0,
    unmut: 0,
    up: 0,
    updat: 0,
    uptim: 0,
    us: 0,
    usag: 0,
    user: 0,
    userinfo: 0,
    valu: 0,
    voic: 0,
    voice_channel: 0,
    voice_tim: 0,
    voice_time_count: 0,
    w_snack: 0,
    w_t: 0,
    want: 1,
    warn: 0,
    watch_togeth: 0,
    watchtogeth: 0,
    what: 0,
    when: 0,
    where: 0,
    word: 0,
    word_snack: 0,
    xp: 0,
    xp_chan: 0,
    xp_channel: 0,
    xp_gain_channel: 0,
    you: [0, 1],
    your: [0, 1],
    yourself: 0,
    youtub: 0,
    youtube_togeth: 0,
    yt_togeth: 0,
    ytt: 0,
  },
  titles: ["Commands", "Welcome to Omnitron\u2019s documentation!"],
  titleterms: {
    avail: 1,
    command: [0, 1],
    contact: 1,
    dj: 0,
    document: 1,
    featur: 1,
    group: 0,
    list: 1,
    miscellan: 0,
    moder: 0,
    omnitron: 1,
    s: [0, 1],
    welcom: 1,
  },
});
